        ...
        getVariantBuilder(BCSiliconBlocks.laser.get()).forAllStates(state ->
                {
                    switch (state.getValue(BCSiliconBlocks.laser.get().getFacingProperty())) {
                        case UP:
                            return ConfiguredModel.builder().modelFile(laser).build();
                        case DOWN:
                            return ConfiguredModel.builder().modelFile(laser).rotationX(180).build();
                        case NORTH:
                            return ConfiguredModel.builder().modelFile(laser).rotationX(90).build();
                        case EAST:
                            return ConfiguredModel.builder().modelFile(laser).rotationX(90).rotationY(90).build();
                        case SOUTH:
                            return ConfiguredModel.builder().modelFile(laser).rotationX(90).rotationY(180).build();
                        case WEST:
                            return ConfiguredModel.builder().modelFile(laser).rotationX(90).rotationY(270).build();
                        default:
                            throw new RuntimeException("Unexpected facing!");
                    }
                }
        );
