...
public class PipeFlowFluids extends PipeFlow implements IFlowFluid, IDebuggable {
    ...
    private static final ActionResult<FluidStack> FAILED_EXTRACT = new ActionResult<>(ActionResultType.FAIL, null);
    private static final ActionResult<FluidStack> PASSED_EXTRACT = new ActionResult<>(ActionResultType.PASS, null);
    ...
    public PipeFlowFluids(IPipe pipe, CompoundNBT nbt) {
        super(pipe, nbt);
        for (EnumPipePart part : EnumPipePart.VALUES) {
            sections.put(part, new Section(part));
        }
        ...
        for (EnumPipePart part : EnumPipePart.VALUES) {
            ...
            if (nbt.contains("tank[" + direction + "]")) {
                CompoundNBT compound = nbt.getCompound("tank[" + direction + "]");
...
//        return tryExtractFluidInternal(millibuckets, from, extractor, simulate).getObject();
    ...
    @Override
//    public ActionResult<FluidStack> tryExtractFluidAdv(int millibuckets, Direction from, IFluidFilter filter, boolean simulate)
    public ActionResult<FluidStack> tryExtractFluidAdv(int millibuckets, Direction from, IFluidFilter filter, IFluidHandler.FluidAction action) {
        FluidExtractor extractor = (mb, c, handler) ->
        {
            if (c != null) {
                if (!filter.matches(c)) {
...
//        return tryExtractFluidInternal(millibuckets, from, extractor, simulate);
    ...
    // private ActionResult<FluidStack> tryExtractFluidInternal(int millibuckets, Direction from, FluidExtractor extractor, boolean simulate)
    private ActionResult<FluidStack> tryExtractFluidInternal(int millibuckets, Direction from, FluidExtractor extractor, IFluidHandler.FluidAction action) {
...
//        if (!simulate)
        ...
        return new ActionResult<>(ActionResultType.SUCCESS, toAdd);
    }
...
//        left.add(" - FluidType = " + (fluid == null ? "empty" : fluid.getDisplayName()));
        left.add(new StringTextComponent(" - FluidType = ").append(fluid == null ? new StringTextComponent("empty") : fluid.getDisplayName()));
...
//            StringBuilder line = new StringBuilder(" - " + LocaleUtil.localizeFacing(part.face) + " = ");
            IFormattableTextComponent firstPart = new StringTextComponent(" - ").append(LocaleUtil.localizeFacingComponent(part.face)).append(new StringTextComponent(" = "));
...
//            left.add(line.toString());
            left.add(firstPart.append(new StringTextComponent(line.toString())));
        }
    }
    ...
    @OnlyIn(Dist.CLIENT)
    public Vector3d[] getOffsetsForRender(float partialTicks) {
        Vector3d[] arr = new Vector3d[7];
        for (EnumPipePart part : EnumPipePart.VALUES) {
            Section s = sections.get(part);
    ...
    @Override
    public void writePayload(int id, PacketBuffer buf, Dist side) {
...
//    public void readPayload(int id, PacketBuffer buf, Dist side) throws IOException
    public void readPayload(int id, PacketBuffer buf, NetworkDirection side) throws IOException {
        PacketBufferBC buffer = PacketBufferBC.asPacketBufferBc(buf);
    ...
    class Section implements IFluidHandler {
        ...
        Vector3d offsetLast, offsetThis;
            ...
            if (part.face == null) {
                ...
                for (EnumPipePart p : EnumPipePart.FACES) {
                    ...
                    if (s.ticksInDirection > 0) {
                        dir = dir.add(Vector3d.atLowerCornerOf(p.face.getNormal()));
                    }
                }
                ...
                for (EnumPipePart p : EnumPipePart.FACES) {
                    Section s = sections.get(p);
                    if (s.ticksInDirection < 0) {
                        dir = dir.add(Vector3d.atLowerCornerOf(p.face.getNormal()).scale(-1));
                    }
                }
                dir = new Vector3d(Math.signum(dir.x), Math.signum(dir.y), Math.signum(dir.z));
