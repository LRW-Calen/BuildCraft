...
public class TilePipeHolder extends TileBC_Neptune implements IPipeHolder, IDebuggable {
    ...
    private CompoundNBT unknownData;

    public TilePipeHolder() {
        super(BCTransportBlocks.pipeHolderTile.get());
    ...
    public CompoundNBT save(CompoundNBT nbt) {
        ...
        CompoundNBT plugs = new CompoundNBT();
        for (Direction face : Direction.values()) {
            CompoundNBT plugTag = pluggables.get(face).writeToNbt();
            if (!plugTag.isEmpty()) {
    ...
    public void load(BlockState state, CompoundNBT nbt) {
        ...
        CompoundNBT plugs = nbt.getCompound("plugs");
        for (Direction face : Direction.values()) {
    ...
    public void onPlacedBy(LivingEntity placer, ItemStack stack) {
        ...
        if (item instanceof IItemPipe) {
            IItemPipe itemPipe = (IItemPipe) item;
...
//    public void getDebugInfo(List<String> left, List<String> right, Direction side)
    public void getDebugInfo(List<ITextComponent> left, List<ITextComponent> right, Direction side) {
        if (pipe == null) {
//            left.add("Pipe = null");
            left.add(new StringTextComponent("Pipe = null"));
        } else {
//            left.add("Pipe:");
            left.add(new StringTextComponent("Pipe:"));
            pipe.getDebugInfo(left, right, side);
        }
//        left.add("Parts:");
        left.add(new StringTextComponent("Parts:"));
        wireManager.parts
//                .forEach((part, color) -> left.add(" - " + part + " = " + color + " = " + wireManager.isPowered(part)));
                .forEach((part, color) -> left.add(new StringTextComponent(" - " + part + " = " + color + " = " + wireManager.isPowered(part))));
//        left.add("All wire systems in world count = " + (level.isClientSide ? 0 : wireManager.getWireSystems().wireSystems.size()));
        left.add(new StringTextComponent("All wire systems in world count = " + (level.isClientSide ? 0 : wireManager.getWireSystems().wireSystems.size())));
        if (unknownData != null) {
//            left.add(unknownData.toString());
            left.add(new StringTextComponent(unknownData.toString()));
        }
    }
...
//    }
    ...
    public ITextComponent getDisplayName() {
        ...
        return new TranslationTextComponent("item." + TagManager.getTag(tagId, TagManager.EnumTagType.UNLOCALIZED_NAME) + ".name");
    }

    @Nullable
    @Override
    public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
        if (this.pipe.behaviour instanceof PipeBehaviourDiamond) {
            PipeBehaviourDiamond diamond = (PipeBehaviourDiamond) this.pipe.behaviour;
            return new ContainerDiamondPipe(BCTransportMenuTypes.PIPE_DIAMOND, id, player, diamond);
        } else if (this.pipe.behaviour instanceof PipeBehaviourWoodDiamond) {
            PipeBehaviourWoodDiamond woodDiamond = (PipeBehaviourWoodDiamond) this.pipe.behaviour;
            return new ContainerDiamondWoodPipe(BCTransportMenuTypes.PIPE_DIAMOND_WOOD, id, player, woodDiamond);
        } else if (this.pipe.behaviour instanceof PipeBehaviourEmzuli) {
            PipeBehaviourEmzuli emzuli = (PipeBehaviourEmzuli) this.pipe.behaviour;
