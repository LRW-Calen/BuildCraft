...
public abstract class SnapshotBuilder<T extends ITileForSnapshotBuilder> implements INBTSerializable<CompoundNBT> {
...
//    };
    ...
    public Vector3d robotPos = null;
    public Vector3d prevRobotPos = null;
...
//            tile.getWorldBC().removeEventListener(worldEventListener);
    ...
    public Vector3d getPlaceTaskItemPos(PlaceTask placeTask) {
//        Vector3d height = new Vector3d(placeTask.pos.subtract(tile.getBuilderPos()));
        Vector3d height = Vector3d.atLowerCornerOf(placeTask.pos.subtract(tile.getBuilderPos()));
        double progress = placeTask.power * 1D / placeTask.getTarget();
//        return new Vector3d(tile.getBuilderPos())
        return Vector3d.atLowerCornerOf(tile.getBuilderPos())
                .add(height.scale(progress))
                .add(new Vector3d(0, Math.sin(progress * Math.PI) * (Math.abs(height.y) + 1), 0))
                .add(new Vector3d(0.5, 1, 0.5));
    }
...
//                        .map(Vector3d::new)
                ...
                newRobotPos = newRobotPos.add(new Vector3d(0, 3, 0));
                Vector3d oldRobotPos = robotPos;
...
//                    tile.getWorldBC().sendBlockBreakProgress(
    ...
    public class BreakTask {
        ...
        @SuppressWarnings("WeakerAccess")
        public BreakTask(CompoundNBT nbt) {
...
//                }
        ...
        @SuppressWarnings("WeakerAccess")
        public PlaceTask(CompoundNBT nbt) {
            pos = NBTUtil.readBlockPos(nbt.getCompound("pos"));
