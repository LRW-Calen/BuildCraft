...
//        BlockUtil.uniqueBlockPosComparator(Comparator.comparingDouble(p -> getBlockPos().distanceSq(p)))
    ...
    public Vector3d drillPos;
    public Vector3d clientDrillPos;
    public Vector3d prevClientDrillPos;
    ...
    private Vector3d collisionDrillPos;
...
//        @Override
//        public boolean handleGameEvent(World p_157846_, GameEvent event, @Nullable Entity p_157848_, BlockPos p_157849_) {
//            return event.;
//        }
//
//        @Override
...
//    };
    ...
    private final ILocalBlockUpdateSubscriber subscriber = new ILocalBlockUpdateSubscriber() {
        ...
        @Override
        public void setWorldUpdated(IWorld w, BlockPos updatePos) {
            if (!(w instanceof World)) {
                return;
            }

            ((World) w).getProfiler().push("bc_quarry_listener");
            if (frameBox.isInitialized() && miningBox.isInitialized()) {
                if (frameBox.contains(updatePos)) {
                    check(updatePos);
                } else if (miningBox.contains(updatePos) && boxIterator != null) {
            ...
            ((World) w).getProfiler().pop();
        }
    };

    public TileQuarry() {
        super(BCBuildersBlocks.quarryTile.get());
    ...
    public void onPlacedBy(LivingEntity placer, ItemStack stack) {
        ...
        TileEntity tile = level.getBlockEntity(areaPos);
...
//                drillPos = new Vector3d(miningBox.closestInsideTo(worldPosition));
                drillPos = Vector3d.atLowerCornerOf(miningBox.closestInsideTo(worldPosition));
            }
...
//                    if (drillPos.squareDistanceTo(new Vector3d(boxIterator.getCurrent())) >= 1)
                    if (drillPos.distanceToSqr(Vector3d.atLowerCornerOf(boxIterator.getCurrent())) >= 1) {
//                        currentTask = new TaskMoveDrill(drillPos, new Vector3d(boxIterator.getCurrent()));
                        currentTask = new TaskMoveDrill(drillPos, Vector3d.atLowerCornerOf(boxIterator.getCurrent()));
                        found = true;
...
//    public List<AxisAlignedBB> getCollisionBoxes()
    public List<VoxelShape> getCollisionBoxes() {
        if (drillPos != null && drillPos != collisionDrillPos && frameBox.isInitialized()) {
            Vector3d max = VecUtil.convertCenter(frameBox.max());
            Vector3d min = VecUtil.replaceValue(VecUtil.convertCenter(frameBox.min()), Axis.Y, max.y);
            collisionBoxes = ImmutableList.of(
...
//        if (drillPos != null && drillPos.squareDistanceTo(new Vector3d(getBlockPos())) > 1024 * 1024)
        if (drillPos != null && drillPos.distanceToSqr(Vector3d.atLowerCornerOf(getBlockPos())) > 1024 * 1024) {
            drillPos = null;
        }
...
//        left.add("drill = " + drillPos);
        ...
        left.add(new StringTextComponent("rate = ").append(LocaleUtil.localizeMjFlowComponent(debugPowerRate)));
        left.add(new StringTextComponent("frameBox"));
        left.add(new StringTextComponent(" - min = " + frameBox.min()));
        left.add(new StringTextComponent(" - max = " + frameBox.max()));
        left.add(new StringTextComponent("miningBox:"));
        left.add(new StringTextComponent(" - min = " + miningBox.min()));
        left.add(new StringTextComponent(" - max = " + miningBox.max()));

        left.add(new StringTextComponent("firstCheckedPoses = " + firstCheckedPoses.size()));
        left.add(new StringTextComponent("frameBoxPosesCount = " + frameBoxPosesCount));
        left.add(new StringTextComponent("firstChecked = " + firstChecked));

        BoxIterator iter = boxIterator;
        left.add(new StringTextComponent("current = " + (iter == null ? "null" : iter.getCurrent())));

        Task task = currentTask;
        if (task != null) {
            left.add(new StringTextComponent("task:"));
            left.add(new StringTextComponent(" - class = " + task.getClass().getName()));
            left.add(new StringTextComponent(" - power = " + LocaleUtil.localizeMjComponent(task.power)));
            left.add(new StringTextComponent(" - target = " + LocaleUtil.localizeMjComponent(task.getTarget())));
        } else {
            left.add(new StringTextComponent("task = null"));
        }
        left.add(new StringTextComponent("drill = " + drillPos));
    }
...
//            level.sendBlockBreakProgress(breakPos.hashCode(), breakPos, -1);
            ...
            Optional<List<ItemStack>> stacks = BlockUtil.breakBlockAndGetDrops(
                    (ServerWorld) level, breakPos, new ItemStack(Items.DIAMOND_PICKAXE), getOwner(), true
            );
            if (stacks.isPresent()) {
    ...
    public class TaskAddFrame extends Task {
        ...
        protected boolean finish(long added, long target) {
            ...
            level.setBlock(framePos, BCBuildersBlocks.frame.get().defaultBlockState(), BlockConstants.UPDATE_ALL);
            return true;
        }
    ...
    private class TaskMoveDrill extends Task {
        public Vector3d from = Vector3d.ZERO;
        public Vector3d to = Vector3d.ZERO;

        TaskMoveDrill() {
        }

        TaskMoveDrill(Vector3d from, Vector3d to) {
            this.from = from;
            this.to = to;
        }
