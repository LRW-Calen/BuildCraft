...
//    public static void renderItemStack(double x, double y, double z, ItemStack stack, int lightc, Direction dir, BufferBuilder bb)
    public static void renderItemStack(ItemStack stack, int lightc, Direction dir, MatrixStack poseStack, IVertexBuilder bb) {
//        renderItemStack(x, y, z, stack, stack.getCount(), lightc, dir, bb);
        renderItemStack(stack, stack.getCount(), lightc, dir, poseStack, bb);
    }

    /** Used to render a lot of items in sequential order. Assumes that you don't change the glstate inbetween calls. */
//    public static void renderItemStack(double x, double y, double z, ItemStack stack, int stackCount, int lightc, Direction dir, MatrixStack poseStack, IVertexBuilder bb)
    public static void renderItemStack(ItemStack stack, int stackCount, int lightc, Direction dir, MatrixStack poseStack, IVertexBuilder bb) {
        if (stack.isEmpty()) {
            return;
        }
        try {
...
//            category.addDetail("Item NBT", () -> "" + stack.getTagCompound());
    ...
    private static void renderItemStackInternal(ItemStack stack, int stackCount, int lightc, Direction dir, MatrixStack poseStack, IVertexBuilder bb) {
        if (dir == null) {
            dir = Direction.EAST;
        }
...
//            bb.setTranslation(0, 0, 0);
        ...
        // Calen
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemCameraTransforms.TransformType.GROUND, lightc, OverlayTexture.NO_OVERLAY, poseStack, Minecraft.getInstance().renderBuffers().bufferSource());
        }
...
//                seed = (regNameSeed & 0x7F_FF_FF_FF) | (((long) stack.getMetadata()) << 32);
    ...
    public static void endItemBatch(MatrixStack poseStack) {
        if (inBatch) {
            inBatch = false;
//            GL11.glPopMatrix();
            poseStack.popPose();
