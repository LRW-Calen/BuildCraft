...
public class TileHeatExchange extends TileBC_Neptune implements ITickable, IDebuggable {
    ...
    public TileHeatExchange() {
        super(BCFactoryBlocks.heatExchangeTile.get());
    }
    ...
    public void load(BlockState state, CompoundNBT nbt) {
        ...
        CompoundNBT nbtSection = nbt.getCompound("section");
    ...
    private Deque<TileHeatExchange> findAdjacentExchangers() {
        ...
        exchangers.add(this);
        for (int i = 1; i < 6; i++) {
            TileEntity neighbour = getLocalTile(worldPosition.relative(dirToStart, i));
        ...
        for (int i = 1; i < 6; i++) {
            TileEntity neighbour = getLocalTile(worldPosition.relative(dirToEnd, i));
            if (neighbour instanceof TileHeatExchange) {
                TileHeatExchange other = (TileHeatExchange) neighbour;
                if (other.getFacing() != thisFacing) {
                    break;
    ...
    public void writePayload(int id, PacketBufferBC buffer, Dist side) {
        if (side == Dist.DEDICATED_SERVER) {
            if (id == NET_RENDER_DATA) {
                ...
                World exchangeWorld = this.level;
                ...
                if (state.getBlock() instanceof BlockHeatExchange) {
                    BlockHeatExchange heatExchangeBlock = (BlockHeatExchange) state.getBlock();
...
//    public <T> T getCapability(@Nonnull Capability<T> capability, Direction facing)
    ...
    public ActionResultType onActivated(
            PlayerEntity player, Hand hand, Direction facing, float hitX, float hitY, float hitZ
    ) {
        if (section != null) {
...
//        super.validate();
    ...
    /** Called by {@link Block#rotate(BlockState, Rotation)} (World, BlockPos, Direction)} and
...
//            left.add("section = null");
            left.add(new StringTextComponent("section = null"));
        } else {
//            left.add("section = " + (section instanceof ExchangeSectionStart ? "start" : "end"));
            left.add(new StringTextComponent("section = " + (section instanceof ExchangeSectionStart ? "start" : "end")));
    ...
    static abstract class ExchangeSection {
        ...
        ExchangeSection(TileHeatExchange tile, CompoundNBT nbt) {
            this(tile);
...
//            left.add("tank_input = " + tankInput.getDebugString());
            left.add(new StringTextComponent("tank_input = " + tankInput.getDebugString()));
//            left.add("tank_output = " + tankOutput.getDebugString());
            left.add(new StringTextComponent("tank_output = " + tankOutput.getDebugString()));
//            left.add("smoothed_input: ");
            left.add(new StringTextComponent("smoothed_input: "));
            smoothedTankInput.getDebugInfo(left, right, side);
//            left.add("smoothed_output: ");
            left.add(new StringTextComponent("smoothed_output: "));
    ...
    public static class ExchangeSectionStart extends ExchangeSection {
        ...
        ExchangeSectionStart(TileHeatExchange tile, CompoundNBT nbt) {
            super(tile, nbt);
...
//            int h_out_amount = h_out_f == null ? max_amount : h_out.fillInternal(h_out_f, false);
        ...
        private void spawnParticles() {
            if (progressState == EnumProgressState.RUNNING) {
                ...
                Vector3d from = VecUtil.convertCenter(getTile().getBlockPos());
        ...
        private void spewForth(Vector3d from, Direction dir, BasicParticleType particle) {
//            Vector3d vecDir = new Vector3d(dir.getDirectionVec());
            Vector3d vecDir = Vector3d.atLowerCornerOf(dir.getNormal());
            ...
            Vector3d motion = VecUtil.scale(vecDir, 0.4);
            int particleCount = Minecraft.getInstance().options.particles.getId();
            World w = getTile().getLevel();
...
//            FluidStack f2 = t.drainInternal(fluid, true);
        ...
        private IFluidHandler getFluidAutoOutputTarget() {
            ...
            TileEntity neighbour = getTile().getNeighbourTile(facing.getClockWise());
            if (neighbour == null) {
                return null;
            }
...
//            // left.add("coolingProvided = " + coolingProvided);
            ...
            left.add(new StringTextComponent("state = " + progressState));
            left.add(new StringTextComponent("has_end = " + (endSection != null)));
            // left.add(new StringTextComponent("heatProvided = " + heatProvided));
            // left.add(new StringTextComponent("coolingProvided = " + coolingProvided));
        }
    }
    ...
    public static class ExchangeSectionEnd extends ExchangeSection {
        ...
        ExchangeSectionEnd(TileHeatExchange tile, CompoundNBT nbt) {
            super(tile, nbt);
        }
...
//            return BuildcraftRecipeRegistry.refineryRecipes.getCoolableRegistry().getRecipeForInput(fluid) != null;
        ...
        IFluidHandler getFluidAutoOutputTarget() {
            TileEntity neighbour = getTile().getNeighbourTile(Direction.UP);
            if (neighbour == null) {
                return null;
            }
